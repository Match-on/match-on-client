...
const RowComponent = ({ row, index, select, info }) => {
  ...
  return (
    <Tablerow
      {...row.getRowProps()}
      onClick={() => router.push(`/classboard/${info.lectureIdx}/${info.lecturePostIdx}`)}
...
const ClassTable = ({ columns, data, handleInputOpen, lectureIdx }) => {
            ...
            {rows.map((row, index) => {
              ...
              return (
                <RowComponent
                  ...
                  select={true}
                  key={`row-${index}`}
                  info={[lectureIdx, data.lecturePostIdx]}
                />
              );
            })}
          </Tbody>

...
import PostMenu from "../../../components/sub/PostMenu";
import ShowComment from "../../../components/sub/ShowComment";
import ResumeModal from "../../../components/sub/ResumeModal";
import ResumeList from "../../../components/sub/ResumeList";
...
const PostContent = ({ type = "team" }) => {
  const router = useRouter();
  const { contestPostIdx } = router.query;
  ...
  const getPostContent = async () => {
    try {
      const response = await axios.get(
        API_URL + `activities/posts/${contestPostIdx}`,
        {
          headers: {
  ...
  const postComment = async () => {
    try {
      const response = await axios.post(
        API_URL + `activities/posts/${contestPostIdx}/comments`,
  ...
  const patchComment = async () => {
    try {
      const res = await axios.patch(
        API_URL + `activities/posts/comments/${commentState.idx}`,
        {
          comment: reply,
        },
        {
      ...
      <PostContainer type={type} isMe={detail.isMe}>
        <Content>
          <Detail>
            <div className="left">
              <Info>
                <div className="nickname">{detail.title}</div>
